...
const DEFAULT_AVAIL: AvailDay[] = DAYS.map((d) => ({
  day: d.value,
...
export default function CreateEventTypeModal({ onClose, onCreated }: Props) {
  const [name, setName] = useState("")
  const [description, setDescription] = useState("")
  const [duration, setDuration] = useState(30)
  const [price, setPrice] = useState("")
  const [color, setColor] = useState("blue")
  const [availability, setAvailability] = useState<AvailDay[]>(DEFAULT_AVAIL)
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex : Appel découverte 30min"
                ...
                {DAYS.map((d) => {
                  ...
                  return (
                    <div key={d.value} className="flex items-center gap-3">
                      ...
                      <span className={`text-xs font-medium min-w-[2.5rem] ${avail.enabled ? "text-gray-700" : "text-gray-400"}`}>
                        {d.label}
                      </span>
                      {avail.enabled && (
                        <div className="flex items-center gap-1.5 flex-1">
                          <input
                            type="time" value={avail.start}
                            onChange={(e) => updateTime(d.value, "start", e.target.value)}
                          ...
                          <input
                            type="time" value={avail.end}
                            onChange={(e) => updateTime(d.value, "end", e.target.value)}
                            className="flex-1 border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-red-500"
                          />
                        </div>
                      )}
                      {!avail.enabled && (
                        <span className="text-xs text-gray-300 flex-1">Indisponible</span>
                      )}
                    </div>
                  )
                })}
                <p className="text-xs text-gray-400 pt-1">
                  Par défaut : Lundi–Vendredi, 9h–17h
                </p>
              </div>
            )}
          </div>
